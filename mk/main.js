...
function selectDevice(name) {
    ...
    suggestionsBox.classList.add('hidden');
    resultDiv.innerHTML = `<span class="text-green-400 font-bold">Да! ${name} е компатибилен.</span>`;
}
...
function createPlanCard(plan) {
    ...
    div.innerHTML = `
        ...
        <div class="text-xl font-semibold mb-4 text-slate-400">${plan.days} Денови</div>
        ...
        <a href="${plan.stripe_link}" class="btn-primary w-full py-3 rounded-xl font-bold text-white uppercase mt-4 text-lg text-center">Купи и активирај сега</a>
        <div class="text-xs mt-3 ${plan.highlight ? 'text-white/80' : 'text-slate-500'}">QR код доаѓа веднаш преку е-пошта</div>
    `;
    return div;
}
