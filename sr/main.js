...
function selectDevice(name) {
    ...
    suggestionsBox.classList.add('hidden');
    resultDiv.innerHTML = `<span class="text-green-400 font-bold">Da! ${name} je kompatibilan.</span>`;
}
...
function createPlanCard(plan) {
    ...
    div.innerHTML = `
        ...
        <div class="text-xl font-semibold mb-4 text-slate-400">${plan.days} Dana</div>
        ...
        <a href="${plan.stripe_link}" class="btn-primary w-full py-3 rounded-xl font-bold text-white uppercase mt-4 text-lg text-center">Kupi sada i aktiviraj</a>
        <div class="text-xs mt-3 ${plan.highlight ? 'text-white/80' : 'text-slate-500'}">QR kod stiže odmah na e-mail</div>
    `;
    return div;
}
